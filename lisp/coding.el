...
;; The following implement functions for the PREDICATE_SYMBOL slot of the
;; coding system types, used by CHECK_CODING_SYSTEM_TYPE(),
;; CONCHECK_CODING_SYSTEM_TYPE() in file-coding.h. Nothing on the C level
;; calls these macros so there is no point in keeping them around in the
;; dumped XEmacs; given this, there is an argument that the PREDICATE_SYMBOL
;; slot should just be removed.
'(
  (defun big5-coding-system-p (object)
    "Return non-nil if OBJECT is a big5 coding-system."
    (and (coding-system-p object) (eq 'big5 (coding-system-type object))))

  (defun shift-jis-coding-system-p (object)
    "Return non-nil if OBJECT is a shift-jis coding-system."
    (and (coding-system-p object) (eq 'shift-jis (coding-system-type object))))

  (defun ccl-coding-system-p (object)
    "Return non-nil if OBJECT is a ccl coding-system."
    (and (coding-system-p object) (eq 'ccl (coding-system-type object))))

  (defun iso2022-coding-system-p (object)
    "Return non-nil if OBJECT is an iso2022 coding-system."
    (and (coding-system-p object) (eq 'iso2022 (coding-system-type object))))

  (defun multibyte-coding-system-p (object)
    "Return non-nil if OBJECT is an multibyte coding-system."
    (and (coding-system-p object) (eq 'multibyte (coding-system-type object))))

  (defun unicode-coding-system-p (object)
    "Return non-nil if OBJECT is a unicode coding-system."
    (and (coding-system-p object) (eq 'unicode (coding-system-type object))))

  (defun internal-coding-system-p (object)
    "Return non-nil if OBJECT is an internal coding-system."
    (and (coding-system-p object) (eq 'internal (coding-system-type object))))

  (defun chain-coding-system-p (object)
    "Return non-nil if OBJECT is a chain coding-system."
    (and (coding-system-p object) (eq 'chain (coding-system-type object))))

  (defun undecided-coding-system-p (object)
    "Return non-nil if OBJECT is an undecided coding-system."
    (and (coding-system-p object) (eq 'undecided (coding-system-type object))))

  (defun convert-eol-coding-system-p (object)
    "Return non-nil if OBJECT is a convert-eol coding-system."
    (and (coding-system-p object)
         (eq 'convert-eol (coding-system-type object))))

  (defun no-conversion-coding-system-p (object)
    "Return non-nil if OBJECT is a no-conversion coding-system."
    (and (coding-system-p object)
         (eq 'no-conversion (coding-system-type object))))

  (when (valid-coding-system-type-p 'mswindows-multibyte)
    (defun mswindows-multibyte-coding-system-p (object)
      "Return non-nil if OBJECT is an mswindows-multibyte coding-system."
      (and (coding-system-p object)
           (eq 'mswindows-multibyte (coding-system-type object))))))
