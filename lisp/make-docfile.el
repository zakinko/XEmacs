...
;; Then parse ../lib-src/DOC, modifying the documentation slots for subr
;; objects, compiled-function objects, and the plists of global variables to
;; reflect offsets into that file.
...
;; A supplied file name is terminated by the ?\037 that commences the next
;; record. doc.c previously accepted termination by ?\n, but support for this
;; has been removed, to simplify implementation of doc string info for
;; anonymous compiled functions.
...
(symbol-macrolet
  ...
  (let*
      ...
      ((source-src (file-name-as-directory
       ...
       ;; We modify these, avoid corruption in our callers. Though this is
       ;; irrelevant with load-history, since it is marked with
       ;; staticpro_dump_nil() to be cleared at dump time, assuming we have
       ;; copied all the relevant information to DOC.
...
than `build-directory' if appropriate. "
	 ...
	 (quote-for-DOC (string)
	   ...
	   (let ((position (position ?\001 string)))
	     (if position
		 (let ((stream (make-string-output-stream)) (last 0))
		   (while position
		     (write-sequence string stream
				     :start last :end position)
                     (incf position)
                     (if (eql position (length string))
                         (setf last position
                               position nil)
                       (case (aref string position)
                         (?\001
                          (write-char ?\001 stream))
                         (?_
                          (write-char ?\037 stream)))
                       (setf last (1+ position)
                             position (position ?\001 string :start last))))
		   (write-sequence string stream :start last)
		   (get-output-stream-string stream))
	       string)))

         (read-symbol-soft (string &optional default)
           ;; Read an existing symbol name, as generated by #'prin1, from
           ;; STRING.  If the corresponding symbol does not reflect an
           ;; interned symbol, return DEFAULT.
           ;; Mainly here to handle #'prin1-to-string adding backslashes, but
           ;; can be extended to handle package syntax in the future.
           ;; #### Note that help.el currently doesn't even attempt to handle
           ;; any of this.
           (if (and (> (length string) 1) (eql (aref string 0) ?#))
               (if (eql (aref string 1) ?#)
                   '## ;; The interned symbol with the zero-length name.
		  default) ;; Uninterned symbol.
             (let ((position (position ?\\ string)))
               (when position
                 (let ((stream (make-string-output-stream)) (last 0))
                   (while position
                     (write-sequence string stream :start last :end position)
                     (incf position)
                     (when (and (< position (length string))
                                (eql (aref string position) ?\\))
                       (write-char ?\\ stream)
                       (incf position))
                     (setf last position
               ...
               (intern-soft string obarray default))))
         ...
         (scan-C-file (output-stream filename modules coding-system)
           ...
           ;;
           ;; CODING-SYSTEM must be a non-constant symbol (so, not nil, t, a
 ...
 +\\(?:DEFVAR\\|DEFINE[A-Z_]+LISP_OBJECT\\)\\)"
                          ...
                          (compiled-function
                           ...
                           (unless (and (not (stringp
                             ...
                             (set-compiled-function-documentation
                              function offset)))))))
                   (?f
                    (setq alist (assoc (unquote-for-DOC
                                        (decode-coding-string
                                         (buffer-substring
                                          (+ (match-beginning 1)
                                             (length "\037S"))
	   ...
	   (maphash
		...
		(unless (or (fixnump (compiled-function-documentation
                                      compiled-function))
			    ;; Uninterned symbol, still reachable from
			    ;; the global value of load-history
			    ;; (e.g. bq-comma from backquote.el):
			    (and (symbolp annotation)
				 (not (eq annotation
					  (intern-soft annotation nil 0))))
			    ;; We don't care about functions in
			    ;; make-docfile.elc, they will not be in
			    ;; the dumped XEmacs.
			    (not (assoc (canonicalize-file-name-for-output
					 annotation)
					load-history)))
                  (format-into 'external-debugging-output
                               "Warning: doc lost for function %S.\n"
		  ...
		  (write-sequence ";\n" output-stream)))
		    ...
		    (write-line (quote-for-DOC (prin1-to-string symbol))
               ...
               (dolist (elt (cdr
                 ...
                 (when (setq documentation
                             (compiled-function-documentation elt))
                   (write-sequence (quote-for-DOC documentation)
                                   output-stream)))))
             load-history))
