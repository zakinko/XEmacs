...
(defun init-windows-glyph-list-4 ()
  ...
  ;; We don't want the characters in Windows Glyph List 4 (and a few others) to
  ;; map to East Asian character sets when they are displayed.  WGL4 is a
  ;; character repertoire from Microsoft that gives a guideline for font
  ;; implementors as to what characters are sufficient for pan-European
  ;; support.  East Asian fonts generally clash strongly with European
  ;; characters both in font choice and character width, and since the WGL 4
  ;; characters are basically European characters, we want them displayed with
  ;; European fonts.  So create a special charset `windows-glyph-list-4' that
  ;; we put higher in the default Unicode precedence list than any Asian
  ;; charsets. (Under old-Mule, these characters will also end up in
  ;; jit-ucs-charset-0, which likewise we put higher than the Asian charsets.)
  ;; 
  (loop for (ucs ascii-or-latin-1)
    in '((#x2013 ?-) ;; U+2013 EN DASH
	 ...
	 ;; These are not in WGL 4, but are IPA characters that should not be
	 ;; double width. They are the only IPA characters that both occur in
	 ;; packages/mule-packages/leim/ipa.el and end up in East Asian
	 ;; character sets when that file is loaded in an XEmacs without
	 ;; packages.
    ...
    ;; entries because it is run at dump time, before we create any charsets
    ;; containing Asian characters, or more specifically before we load the
    ;; Unicode mapping tables for the Asian-character charsets, which currently
    ;; happens at charset creation time. (That happens in mule-charset.el.) It
    ;; doesn't matter whether we move the call to (set-unicode-conversion code
