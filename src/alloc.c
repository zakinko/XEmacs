...
static const struct memory_description lrecord_implementation_description_1[] = {
  ...
  { XD_FUNCTION_POINTER, offsetof (struct lrecord_implementation,
                                   memory_usage) },
  { XD_LISP_OBJECT, offsetof (struct lrecord_implementation,
			      memusage_stats_list) },
...
/* Mark the Lisp_Object at VARADDRESS as a root object for garbage collection,
   ...
   made known to the dumper usually by means of
   dump_add_root_block_ptr(). Pointers to Lisp_Objects within blocks handed to
   the dumper by means of dump_add_root_block() and by means of staticpro()
   are handled correctly. There will usually be no need for the latter.
		    ...
		    enum lrecord_alloc_status status)
{
  ...
  switch (status)
	...
	if (HAS_OBJECT_METH_P (obj, memory_usage))
	  {
	    Elemcount i;
	    Elemcount total_stats
	      = OBJECT_PROPERTY (obj, num_extra_memusage_stats);
	    xzero (stats);
