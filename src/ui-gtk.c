...
typedef struct
{
  /* The low-order sixteen bits of the hash of the type info for the function
     (so, the hash of "RETURN-TYPE__ARG1-TYPE_ARG2-TYPE_...".

     We take the low-order sixteen bits of the hash of the string type info
     because this avoids 32-64 bit issues with the size of Hashcode. Happily,
     it also gives a lot of variation. */
  const UINT_16_BIT type_string_hash;
  /* Pointer to the corresponding marshaller function, auto-generated by
     gtk-marshal.el. */
  const ffi_marshalling_function marshaller;
} marshaller_map_elt;

/* An auto-generated file of marshalling functions. See
   ../lisp/gtk-marshal.el. */
#include "emacs-marshals.c"
#undef MANY_ARGS

#define FROB(type_info, function)                       \
  {type_info, (ffi_marshalling_function) function},
#define FROB_LAST(type_info, function)                  \
  {type_info, (ffi_marshalling_function) function}

/* A map from the hash of the type string to the corresponding marshaller,
   ordered numerically by the hash. This ordering means we can use binary
   search for the hash, avoiding the issue that the string type info does not
   have the variance necessary to make this quick. */
static const marshaller_map_elt gtk_marshaller_type_map[] =
{
  FOR_ALL_MARSHALLERS (FROB, FROB_LAST)
};
...
static int
marshaller_compare (const void *a, const void *b)
{
  return *((const UINT_16_BIT *) a) - *((const UINT_16_BIT *) b);
}

static ffi_marshalling_function
find_marshaller (Lisp_Object type_details)
{
  UINT_16_BIT type_hash
    = (UINT_16_BIT) ((hash_string (XSTRING_DATA (type_details),
				   XSTRING_LENGTH (type_details)) & 0xFFFF));
  marshaller_map_elt *result
    = (marshaller_map_elt *) bsearch (&type_hash,
                                      gtk_marshaller_type_map,
                                      countof (gtk_marshaller_type_map),
                                      sizeof (gtk_marshaller_type_map[0]),
				      marshaller_compare);
  if (result != NULL)
    {
       ...
       (rettype, name, args))
{
  ...
  marshaller_func = find_marshaller (marshaller);
