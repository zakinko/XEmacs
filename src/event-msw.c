 ...
 */
static Lisp_Object Vmswindows_s_dispatch_event_queue;
static Lisp_Object Vmswindows_s_dispatch_event_queue_tail;
...
mswindows_enqueue_dispatch_event (Lisp_Object event)
{
  ...
  if (user_p)
    ...
    enqueue_event (event, &Vmswindows_s_dispatch_event_queue,
...
{
  assert (!NILP (Vdispatch_event_queue) ||
	  !NILP (Vmswindows_s_dispatch_event_queue));

  if (!NILP (Vdispatch_event_queue))
    return dequeue_dispatch_event ();
  else
    return dequeue_event (&Vmswindows_s_dispatch_event_queue,
			  &Vmswindows_s_dispatch_event_queue_tail);
}

#ifndef CYGWIN
...
mswindows_need_event_in_modal_loop (int badly_p)
{
  ...
  while (NILP (Vmswindows_s_dispatch_event_queue))
    {
	...
	if (ii)
	  {
	    ...
	    image_instance = GET_LISP_FROM_VOID ((const void *) ii);
...
mswindows_find_frame (HWND hwnd)
{
  ...
  f = GET_LISP_FROM_VOID ((const void *) l);
  return f;
}
...
{
  Vmswindows_s_dispatch_event_queue = Qnil;
  staticpro (&Vmswindows_s_dispatch_event_queue);
  Vmswindows_s_dispatch_event_queue_tail = Qnil;
  dump_add_root_lisp_object (&Vmswindows_s_dispatch_event_queue_tail);
